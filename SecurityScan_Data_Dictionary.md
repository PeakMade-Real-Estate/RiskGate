...
5. **Same-device suppression** — if the same `deviceId` made both sign-ins, the event is suppressed **before** the 2-of-4 factor check ever runs ([app/routes.py](app/routes.py#L1072)). **As originally written this was unconditional** (suppressed even 15,000 mph international travel). **First fixed** to bypass on `required_speed_mph > 10000` **or** a country change. **Retuned again this session (2026-09-24)**: it now only suppresses when the country is unchanged, **regardless of speed** ([app/routes.py](app/routes.py#L1061, #L1072)) — a country change is the only thing that now bypasses this suppression. Rationale: a remote worker on VPN keeps the same device and stays in-country, but VPN/proxy egress-node hopping routinely produces apparent speeds of 10,000–800,000+ mph between domestic cities; treating raw speed as a bypass signal was flagging legitimate remote workers, not attackers.
6. **Trusted-location suppression** — if either endpoint has been visited 3+ times in the current scan batch ([app/routes.py](app/routes.py#L969-L980)), the event is suppressed. **Same history and same retuning**: originally unconditional, then bypassed on extreme-speed-or-country-change, **now (2026-09-24) only bypassed by a country change** ([app/routes.py](app/routes.py#L1080-L1090)).
...
- **Same-device and trusted-location suppression were unconditional overrides**, suppressing even 15,000 mph international travel. **Fixed** as described in points 5–6 above (then **retuned again 2026-09-24** to a country-change-only bypass, see below).
...
### Remote-worker/VPN retuning (2026-09-24) — live scan evidence + confirmed by direct execution
A live manual scan surfaced dozens of alerts with 0.01–0.08 hour gaps (36 seconds–5 minutes) between domestic sign-ins at speeds of 10,000–830,000 mph — the signature of a VPN/corporate-proxy egress node reporting a different geolocation on consecutive requests from the same physical user, not real travel. Because the prior fix let *any* extreme speed bypass same-device/trusted-location suppression, these VPN jumps were never suppressed and instead reliably scored 2+ risk factors (`new_location` or `different_device` + `extreme_speed`), surfacing as `High Risk`/`Critical`. Changed the bypass condition on both suppression checks from `speed > 10000 OR country changed` to `country changed` only ([app/routes.py](app/routes.py#L1061, #L1072, #L1090)).

**Confirmed by direct execution against synthetic data (2026-09-24, in a live Flask app context, no production data touched):**
- Same device, same country, 850-mile jump in 2 minutes (25,500 mph, simulating VPN egress-node hopping) → **0 alerts generated** (correctly suppressed).
- Same device, country change (Atlanta → London), same 2-minute gap → **1 alert generated**, logged as `ALERT: traveler@test.com - Atlanta → London (126333 mph) Risk factors: new_location, extreme_speed, international` (correctly NOT suppressed).

### Verdict
**MATCHES your stated rule for its 4 named requirements.** The two additional device/trust suppression layers now use a country change (not speed) as the only override signal, which better reflects that VPN/proxy speed anomalies are common for legitimate remote workers while a country change is a stronger compromise indicator. **Both branches confirmed working as intended via direct execution, above.**

---

## Part 4b — MFA-Change Correlation ("CRITICAL - MFA Changed" tag) — bug found and fixed 2026-09-24

Location: `scan_entra()` (the manual `/scan` route), [app/routes.py](app/routes.py#L467).

**Confirmed defect (present until 2026-09-24):** the audit-log ingestion loop built a single tenant-wide `mfa_change_times` set of every scanned user's MFA/authentication-method change timestamps ([app/routes.py](app/routes.py#L663)). The later alert-building loop's `mfa_changed_after` check ([app/routes.py](app/routes.py#L813-L814)) tested each impossible-travel alert's login time against this **global, unfiltered** set — with no check that the MFA change belonged to the *same user* as the login. On a scan of the full "All Associates" group over a 7-day window, some user's MFA is essentially always changing within an hour of any other user's login, so the `CRITICAL - MFA Changed` tag ([app/routes.py](app/routes.py#L830)) was firing on coincidence, not real correlation — this is very likely why a large fraction of alerts in the 2026-09-24 manual scan showed that tag.

**Fix:** added a per-user index `mfa_change_times_by_user` (dict of lowercased user email → set of change datetimes, [app/routes.py](app/routes.py#L664)), populated only after the existing `scanned_user_emails` filter ([app/routes.py](app/routes.py#L705)), and changed the correlation check to look up only the login's own user's MFA-change times ([app/routes.py](app/routes.py#L813-L814)) before deciding `mfa_changed_after`. The unfiltered `mfa_change_times` set is still built ([app/routes.py](app/routes.py#L663)) but is no longer read anywhere for this decision — it remains used only as a fallback empty-set reset on exception ([app/routes.py](app/routes.py#L773)).

**Confirmed by direct execution (2026-09-24):** simulated User A's MFA change and User B's login 70 seconds apart — cross-user lookup returned `flagged: False` (no longer falsely correlated); the same lookup against User A's own login correctly returned `flagged: True` (genuine same-user correlation still works). Both results match the intended fix exactly.

**Known design limitation (confirmed present, by design not yet changed):** the `mfa_keywords` list used to classify an audit event as an MFA change ([app/routes.py](app/routes.py#L685-L687)) includes broad terms like `'registered'`, `'deleted'`, `'updated'`, `'reset'`, `'removed'` — these will also match non-MFA audit activity (e.g., any object update/registration/deletion in the directory). This is a real, confirmed source of potential over-matching, independent of the per-user fix above, and has not been narrowed this session — flag to leadership as a known scope limitation of the `CRITICAL - MFA Changed` tag, not a data-accuracy defect in what's documented here.
...
**Resolved this pass (2026-09-24), via direct Azure query and direct code execution:**
10. `SCHEDULER_TARGET_TYPE` in the deployed App Service's application settings — **confirmed via `az webapp config appsettings list --name RiskGate --resource-group riskgate_group`: set to `group`, not `all_users`.** The `fetch_all_users()` path (§5.7) is implemented and safe, but is not the active scan target in production today.
11. The MFA cross-user correlation fix (Part 4b) and the same-device/trusted-location country-change-only retuning (Part 4) — **both confirmed correct via direct execution against synthetic data** in a live app context (results documented in Part 4 and Part 4b above). No production data was used or modified.

**Confirmed as moot (not an open question, but worth noting for repo hygiene):**
- The two branching Alembic `down_revision = 'c34d5ec874d2'` heads were never reconciled in any meaningful sense **because finding 9 already confirms this production database has no Alembic version table at all** — none of this repo's migrations, on either branch, have ever been applied here. This is a repo/migration-history inconsistency worth cleaning up separately, but it has zero effect on current runtime behavior.

**Known, confirmed, and documented (not uncertain — a stated scope limitation, see Part 4b):**
- The `mfa_keywords` list's broad matching (`'registered'`, `'deleted'`, `'updated'`, `'reset'`, `'removed'`) will match some non-MFA audit activity. This is a confirmed characteristic of the current implementation, not an unverified claim — it has not been narrowed, and is flagged here as a design limitation for leadership awareness.

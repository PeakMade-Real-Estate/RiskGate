...
Verified via `get_errors` (no syntax errors) and app restart; **not yet re-verified against a fresh live scan** to confirm the `CRITICAL - MFA Changed` count drops to only genuinely same-user correlations.

**Separate, unaddressed observation:** the `mfa_keywords` list used to classify an audit event as an MFA change ([app/routes.py](app/routes.py#L685-L687)) includes broad terms like `'registered'`, `'deleted'`, `'updated'`, `'reset'`, `'removed'` — these can match non-MFA audit activity (e.g., any object update/registration/deletion in the directory), which could still cause over-matching independent of the per-user fix above. Not changed this session; flagged for future review if `CRITICAL - MFA Changed` still looks noisy after re-scanning.
...
**Still open — code-fixed 2026-09-24 but not yet re-verified against a fresh live scan (see Part 4 and Part 4b):**
- The MFA cross-user correlation bug (`CRITICAL - MFA Changed` firing on any scanned user's MFA activity, not just the alerted user's own) is fixed in code; the resulting drop in false `CRITICAL - MFA Changed` tags has not yet been confirmed via a re-scan.
- The same-device/trusted-location suppression retuning (country-change-only bypass, replacing the speed-based bypass) is fixed in code; whether this actually reduces the VPN/remote-worker false-positive rate in practice, without also suppressing real cross-country compromise cases, has not yet been confirmed via a re-scan.
- The `mfa_keywords` list's broad matching (`'registered'`, `'deleted'`, `'updated'`, `'reset'`, `'removed'`) is a known potential source of remaining over-matching, not yet investigated or changed.
